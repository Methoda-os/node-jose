  ...
  Object.defineProperty(this, 'final', {
    value: function (data, encoding) {
              ...
              p = encKey.then(function (jwk) {
                ...
                if (!encAlg) {
                  props.enc =
                    fields.enc =
                    encAlg =
                      jwk.algorithms(JWK.MODE_ENCRYPT)[0]
                }
                return {
                  once: true,
                  direct: true
          ...
          rpromise = rpromise.then(function (wrapped) {
            if (wrapped.once && !one) {
              return Promise.reject(
                new Error(
                  "cannot use 'alg':'" + algAlg + "' with multiple recipients"
                )
              )
            }
      ...
      promise = promise.then(function (jwe) {
        ...
        if (
          (protectAll && jwe.recipients.length === 1) ||
          'compact' === format
        ) {
...
function createEncrypt (opts, rcpts) {
  ...
  if (arguments.length === 1) {
    ...
    JWK.isKey(opts) ||
    (opts && 'kty' in opts) ||
    (opts && 'key' in opts && (JWK.isKey(opts.key) || 'kty' in opts.key))
  ) {
    rcptList = opts
    rcptStart = 0
    options = {}
  ...
  switch (options.format) {
    case 'compact':
      if ('aad' in opts) {
        throw new Error(
          'additional authenticated data cannot be used for compact serialization'
        )
      }
  ...
  rcptList = rcptList.map(function (r, idx) {
    ...
    p = p.then(function (recipient) {
      ...
      if (!props.alg) {
        return Promise.reject(
          new Error('key not valid for encrypting to recipient ' + idx)
        )
      }
      header.alg = props.alg
